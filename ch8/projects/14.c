...
int main(void)
{
    ...
    */
    char sentenes[70];
    int i = 1, word_count = 0;
    printf("Enter a sentence: ");
    while ((sentenes[0] = getchar()) == ' ')
        ;
    while ((sentenes[i] = getchar()) != '\n')
        i++;
    for (int j = i; j >= -1; j--) {
        if (sentenes[j] == ' ' || j == -1) {
            word_count = j + 1;
            while (sentenes[word_count] != ' ' && sentenes[word_count] != '?' && sentenes[word_count] != '!') {
                printf("%c", sentenes[word_count]);
                word_count++;
            }
            if (j != -1) {
                printf(" ");
